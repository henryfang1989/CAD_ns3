...
//turn on global static routing
	...
	std::vector<double>tcpstart(nLeftNodes);
	std::vector<double>udpstart(nMidNodes);
	std::vector<double>udpstart2(nMidNodes);
//	double stop = 10.0;
	uint32_t tcpPort = 50;
	uint32_t udpPort = 60;
 ...
 // anim. SetConstantPosition (routers.Get(3), 200, 300);
  ...
  for (uint32_t i=0; i<nMidNodes;i++)
  {
    ...
    udpTotalRecv =udpTotalRecv + goodput;
  	...
  	std::cout<<"			average goodput: "<<udpTotalRecv/5<<std::endl<<std::endl;
  ...
  for (uint32_t i=0; i<nLeftNodes;i++)
  {
    ...
    udpTotalRecv2 = udpTotalRecv2 + goodput;
  	...
  	std::cout<<"			average goodput: "<<udpTotalRecv2/5<<std::endl<<"			total average: "<<(tcpTotalRecv+udpTotalRecv + udpTotalRecv2)/15<<std::endl<<std::endl;
