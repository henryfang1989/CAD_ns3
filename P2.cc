...
int main(int argc, char *argv[])
{
	...
	uint32_t segSize = 3000;
	uint32_t windowSize = 5000;
	std::string dataRate = "5Mbps";
	std::string RdataRate = "10Mbps";
...
//turn on global static routing
	...
	uint32_t tcpPort = 50;
	uint32_t udpPort = 60;
//Install on/off app on all left nodes
	OnOffHelper leftClientHelper ("ns3::TcpSocketFactory", Address ());
	leftClientHelper.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
	leftClientHelper.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));

	ApplicationContainer leftClientApps;
	for (uint32_t i=0; i<nLeftNodes; i++)
	{
		AddressValue remoteAddress (InetSocketAddress (rightInterfaceList[i].GetAddress(1), tcpPort));
		leftClientHelper.SetAttribute ("Remote", remoteAddress);
	...
	std::vector<ApplicationContainer> sinkApps (nRightNodes);
	for(uint32_t i=0; i<nRightNodes; i++)
    {
    	if(i<nLeftNodes)
    	{
    		PacketSinkHelper sink ("ns3::TcpSocketFactory",
      		...
      		sinkApps[i] = sink.Install (rightNodes.Get (i));
    	}
    	else
    	{
    		PacketSinkHelper sink ("ns3::UdpSocketFactory",
                           InetSocketAddress (Ipv4Address::GetAny (), tcpPort));
      		sinkApps[i] = sink.Install (midNodes.Get (i - nLeftNodes));
    	}
