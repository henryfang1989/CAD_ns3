...
int main(int argc, char *argv[])
{
	...
	cmd.AddValue ("interPacketInterval", "inter packet interval for udp flow", interPacketInterval);
	cmd.Parse (argc,argv);
...
//turn on global static routing
	...
	std::vector<double>start(nLeftNodes);
//	double stop = 10.0;
	uint32_t tcpPort = 50;
	uint32_t udpPort = 60;
...
//create tcp source
	...
	for (uint32_t i=0; i<nLeftNodes; i++)
	{
		...
		leftClientApps[i].Start (Seconds(start[i]));
...
//create udp source
   	...
   	std::vector<double> udpstart(nMidNodes); 
	...
	for (uint32_t i=0; i<nMidNodes; i++)
	{
		...
		midClientHelper.SetAttribute ("Interval", TimeValue (Seconds(interPacketInterval)));
...
*/
	...
	if(queueType == "droptail")
		{
			...
			std::cout<<"UDP flows: "<<" Interval: "<<interPacketInterval<< " maxPacketSize: "<<maxPacketSize<<std::endl;
		}
	if(queueType =="RED")
		{
			std::cout<<"host to router link: "<<" DataRate: "<<dataRate<<" Delay: "<<delay<<std::endl<<"bottleneck link: "<<" DataRate: "<<RdataRate<<" Delay: "<<Rdelay<<std::endl;
  ...
  for (uint32_t i=0; i<nLeftNodes;i++)
  {
    ...
    double goodput = (sink1->GetTotalRx ())/(10-start[i]);
    std::cout << "TCP goodput: " << goodput <<std::endl;
    tcpTotalRecv = tcpTotalRecv + goodput;       
  }
  	std::cout<<"			average goodput: "<<tcpTotalRecv/5<<std::endl;
  ...
  for (uint32_t i=0; i<nMidNodes;i++)
  {
    ...
    double goodput = (sink2->GetReceived())*maxPacketSize/(10-udpstart[i]);
    std::cout << "UDP goodput: " << goodput <<std::endl; 
    udpTotalRecv =udpTotalRecv + goodput;
  }
  	std::cout<<"			average goodput: "<<udpTotalRecv/5<<std::endl<<"			total average: "<<(tcpTotalRecv+udpTotalRecv)/10<<std::endl<<std::endl;
      Simulator::Destroy ();
}
